...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-yztest";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-yztest list item 1",
        data: {name: "list item 1"}
      },
      {
        label: "coc-yztest list item 2",
...
async function activate(context) {
  import_coc2.window.showMessage(`coc-yztest works!`);
  context.subscriptions.push(import_coc2.commands.registerCommand("coc-yztest.Command", async () => {
    import_coc2.window.showMessage(`coc-yztest Commands works!`);
  }), import_coc2.listManager.registerList(new lists_default(import_coc2.workspace.nvim)), import_coc2.sources.createSource({
    name: "coc-yztest completion source",
    doComplete: async () => {
      const items = await getCompletionItems();
      return items;
    }
  }), import_coc2.workspace.registerKeymap(["n"], "yztest-keymap", async () => {
